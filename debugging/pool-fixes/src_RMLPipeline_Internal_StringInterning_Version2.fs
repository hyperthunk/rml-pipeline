...
//   4. Canonicalisation APIs
//   5. Internal race-handling updates
//
//  Sections clearly marked with PATCH markers for easier review.
...
module StringInterning =
    ...
    /// Promotion policy abstraction (can be mocked in tests)
    ...
    /// Injected promoter (tier redirection)
    type StringPromoter =
        ...
        abstract member promoteNone : string -> StringId  // returns existing / no-op
    ...
    // -----------------------------------------------------------------------------------------
    // Existing config / stats kept (unchanged)
    // -----------------------------------------------------------------------------------------
    ...
    module IdAllocation =
        ...
        let getLocalPoolBaseId (groupId: DependencyGroupId option) (WorkerId w) =
            ...
            | Some gid ->
                ...
                groupBase + GroupPoolRangeSize + (wh * WorkerRangeSize)
            | None ->
                let wh = abs (w.GetHashCode()) % 100000
                LocalPoolBase + (wh * WorkerRangeSize)

    // -----------------------------------------------------------------------------------------
    // HashIndexing (PATCH: overflow removal)
    // -----------------------------------------------------------------------------------------
    module HashIndexing =
        ...
        type StashEntry =
            { mutable key: string
              ...
              mutable state: int32 } // 0=empty 1=used
                        ...
                        if not (isNull k) && k = str then
                            ...
                            | ValueNone ->
                                ...
                                | ValueNone ->
                                    ...
                                    let ring = shard.Ring
                                    let mutable j = 0
                                    let mutable rFound = ValueNone
                                    while rFound = ValueNone && j < ring.Length do
                                        let keyR = Volatile.Read(&ring[j].key)
                                        if not (isNull keyR) then
                                            if ring[j].hashFragment = frag && keyR = str then
                                                rFound <- ValueSome (unpackLocation ring[j].packed)
                                        j <- j + 1
                                    match rFound with
                                    | ValueSome loc ->
    ...
    // -----------------------------------------------------------------------------------------
    // Promotions (unchanged structurally; inline promotions done elsewhere)
    // -----------------------------------------------------------------------------------------
    ...
    module Promotions =
        ...
        // Minimal placeholder queue (kept for compatibility)
    ...
    module Packing =
        ...
        module PackedOps =
            ...
            //   Bits 46-0 : stringIndex
            // If promoted:
            //   Bits 62-0 : canonical ID (positive)
    ...
    module Segments =
        ...
        let inline newChunkSize (s:Segments) =
            if s.Chunks.Count = 1 then max s.Configuration.SecondaryChunkSize s.ChunkSize
            else
                let last = s.Chunks.[s.Chunks.Count-1].PackedEntries.Length
                int (float last * s.Configuration.ChunkGrowthFactor) |> min 50000

        let private grow (s:Segments) =
            let idx = s.CurrentChunkIndex + 1
            if idx < s.Chunks.Count then ()
            else
                let sz = newChunkSize s
                let newC =
                    { Packing.PackedPoolArray.PackedEntries = Array.zeroCreate sz
                      StringsArray = Array.zeroCreate sz
                      NextStringIndex = 0 }
                s.Chunks.Add newC
        ...
        let private allocate (s:Segments) (str:string) =
            ...
            if offset < s.ChunkSize then
                ...
                try
                    ...
                    if readHeld then
                        let idxCheck = Volatile.Read(&s.CurrentChunkIndex)
                        if offset < s.ChunkSize then
                            // another thread resized earlier
                            allocate s str
                        else
        ...
        let allocateOrReuse (s:Segments) (str:string) =
            ...
            | ValueSome loc when loc.ArrayType > 0uy ->
                let segmentIdx = int loc.ArrayType - 1
                if segmentIdx >= s.Chunks.Count then allocate s str else
                let chunk = s.Chunks.[segmentIdx]
                ...
                if Packing.PackedOps.isPromoted packed then
                    ...
                    incrementTempAndMaybePromote s segmentIdx loc.EntryIndex loc.StringIndex packed
            | _ -> allocate s str
    ...
    type Pool =
        { EdenArray: Packing.PackedPoolArray
          ...
          mutable Stats: Stats
          // metrics
          mutable RedirectResolutions: int64
          mutable RedirectCompressed: int64 }
    ...
    module Pool =
        let create baseId edenSize cfg policy promoter =
            { EdenArray =
              ...
              RedirectResolutions = 0L
              RedirectCompressed = 0L }
        ...
        let private edenLookup (p:Pool) (str:string) =
            ...
            | ValueSome loc when loc.ArrayType = HashIndexing.EdenArrayType ->
                ...
                if slot >= 0 && slot < p.EdenArray.PackedEntries.Length then
                    ...
                    if Packing.PackedOps.isPromoted packed then
                        // path compression: we trust promoted entry – returns final
                        Packing.PackedOps.unpackPromoted packed
                    else
        ...
        // Canonicalisation with path compression
            ...
            if id.Value < p.PoolBaseId then id
                ...
                if rel < p.EdenArray.PackedEntries.Length then
                    ...
                    if Packing.PackedOps.isPromoted packed then
                        ...
                        StringId.Create(finalId.Value) // temperature lost intentionally
                    else id
                else
                    // segments
                    let postBase = p.PostEden.BaseId
                    let relSeg = id.Value - postBase
                    if relSeg < 0 then id
                    else
                        // segment chain
                        // (Currently simplest: no deep chain stored beyond one redirect)
                        id
        ...
        let tryGet (p:Pool) (id:StringId) =
            if id.Value < p.PoolBaseId then None
                ...
                if rel < p.EdenArray.PackedEntries.Length then
                    let packed = Volatile.Read(&p.EdenArray.PackedEntries.[rel])
                    if Packing.PackedOps.isPromoted packed then
                        // redirect => upper tier must handle outside (callers cascade)
                        None
                    else
                        let si,_ = Packing.PackedOps.unpackStringIndexAndTemp packed
    ...
    // -----------------------------------------------------------------------------------------
    // Public façade types (Global / Group / Local will wrap these pools in StringPool.fs)
    //  - Provided canonicalise, intern, get APIs.
    // -----------------------------------------------------------------------------------------
